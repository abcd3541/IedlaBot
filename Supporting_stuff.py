...
def Exit(data, is_gemini_history=False, freakseek=True):
    global Freak
    Freak = False
    ...
    if is_gemini_history:
        ...
        file = "Dictionary_storage_Uncensored.json"
...
def save_todo(todo,user):
    todo_storage = f"{user}_todolst.json"
    try:
        with open(todo_storage,'w') as z:
            json.dump(todo,z,indent=4)
            return 'saved'
    except FileNotFoundError:
        with open(todo_storage,"w") as x:
            json.dump(todo,x)
...
def todo_lst(todo : list,user):
    task = [f"{item}"for item in todo]
    todo = "\n".join(task)
    todolist = discord.Embed(type='rich',
        title= 'Task List',
        description=todo,
        color=0x00ff00,
    ...
    todolist.set_author(name=user,icon_url=user.avatar)
    todolist.set_footer(text="made by iedla")

    return todolist

def get_todolst(user):
    todo_storage = f"{user}_todolst.json"
    todo = []
    ...
    try:
        ...
        if not file_content:
            ...
            try:
                todolst = json.loads(file_content)
                print("Successfully loaded JSON file")
            except json.JSONDecodeError as e:
                print("Error. Resetting")
                todolst = todo
                with open(todo_storage, "w") as x:
                    json.dump(todolst, x, indent=4)

        if not isinstance(todolst, list):
            todolst = todo

            with open(todo_storage, "w") as x:
                json.dump(todolst, x, indent=4)

        print(f"get_todolst returning final list: {todolst}")
        return todolst

    except FileNotFoundError:
        with open(todo_storage, "w") as x:
            json.dump(todo, x, indent=4)
        return todo
